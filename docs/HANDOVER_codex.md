...
- **L4**：§5 表中 #2/#3/#4/#5/#6/#7 逐个清零（#2/#4/#6 不依赖 L0 可
  ...
  **fallback == 0**。
- **L4b（融合阶段，L0 落地后立即做，性能的主要来源）**：把正确性
  优先的分解坍缩为融合形态——RMSNorm 5→2 kernel（归约 + 融合尾羽/
  广播/缩放）、attention 链 9→2-3、GDN 读取 2→1、撤销 ones-matmul
  的 128 倍冗余展开、RoPE 融合 pointwise。融合边界：pointwise 链内
  与 epilogue；归约/matmul 天然独立（matmul_acc 是后续扩展点）。
  每坍缩一项重跑家族验收 + D-0017 计时对比前后。
- **L5**：D-0017 性能段：独占 GPU 窗口逐家族计时（policy-2 通道），
  报告里区分“结构性开销 vs 可优化”（§7），并纳入 L4b 的前后对比。
