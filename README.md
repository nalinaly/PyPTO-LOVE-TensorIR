...
| `attention` | 2 | **dense 已通过；paged decode 源码候选** | dense QK→稳定 softmax→PV；decode 单图直接读取 SGLang request table、投影物理 KV rows，并用 GPU seq-len 做 GQA masked softmax；覆盖 0.8B `8Q/2KV` 与 9B `16Q/4KV` 源码几何，仍待 CUDA Tile run-pass；prefill/cache write/连续批处理仍开放 |
