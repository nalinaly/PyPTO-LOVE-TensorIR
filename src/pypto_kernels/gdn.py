...
def state_read_status(heads: int = 16, dk: int = 128, dv: int = 128) -> dict[str, str]:
    return classify(gdn_state_read_graph(heads, dk, dv), tiles_for(heads, dv))


def delta_combine_status(heads: int = 16, dv: int = 128) -> dict[str, str]:
    return classify(gdn_delta_combine_graph(heads, dv), tiles_for(heads, dv))


def state_update_status(
    heads: int = 16, dk: int = 128, dv: int = 128
) -> dict[str, str]:
    return classify(gdn_state_update_graph(heads, dk, dv), tiles_for(heads, dk, dv))
